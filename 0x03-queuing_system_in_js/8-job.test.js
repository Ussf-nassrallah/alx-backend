...
describe('createPushNotificationsJobs', () => {
  ...
  it('adds jobs to the queue with the correct type and logs creation', (done) => {
    ...
    createPushNotificationsJobs(jobDataArray, testQueue);

    expect(testQueue.testMode.jobs.length).to.equal(2);
    expect(testQueue.testMode.jobs[0].data).to.deep.equal(jobDataArray[0]);
    expect(testQueue.testMode.jobs[0].type).to.equal('push_notification_code_3');

    testQueue.process('push_notification_code_3', () => {
      expect(loggerSpy.log.calledWith('Notification job created:', testQueue.testMode.jobs[0].id)).to.be.true;
      done();
    });
  });

  it('registers the progress event handler for a job and logs progress', (done) => {
    testQueue.testMode.jobs[0].addListener('progress', () => {
      expect(loggerSpy.log.calledWith('Notification job', testQueue.testMode.jobs[0].id, '25% complete')).to.be.true;
      done();
    });
    testQueue.testMode.jobs[0].emit('progress', 25);
  });

  it('registers the failed event handler for a job and logs failure', (done) => {
    testQueue.testMode.jobs[0].addListener('failed', () => {
      expect(loggerSpy.log.calledWith('Notification job', testQueue.testMode.jobs[0].id, 'failed:', 'Failed to send')).to.be.true;
      done();
    });
    testQueue.testMode.jobs[0].emit('failed', new Error('Failed to send'));
  });

  it('registers the complete event handler for a job and logs completion', (done) => {
    testQueue.testMode.jobs[0].addListener('complete', () => {
      expect(loggerSpy.log.calledWith('Notification job', testQueue.testMode.jobs[0].id, 'completed')).to.be.true;
      done();
    });
